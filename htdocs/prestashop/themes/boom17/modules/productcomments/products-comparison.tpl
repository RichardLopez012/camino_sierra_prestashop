...
<tr>
	...
	{foreach from=$list_ids_product item=id_product}
		...
		<td  width="{$width|intval}%" class="{$classname|escape:'html':'UTF-8'} comparison_infos ajax_block_product" align="center">
...
{foreach from=$list_ids_product item=id_product}
	<td  width="{$width|intval}%" class="{$classname|escape:'html':'UTF-8'} comparison_infos" align="center" >
	{if isset($list_product_average[$id_product]) AND $list_product_average[$id_product]}
		{section loop=6 step=1 start=1 name=average}
			<input class="auto-submit-star" disabled="disabled" type="radio" name="average_{$id_product|intval}" {if $list_product_average[$id_product]|round neq 0 and $smarty.section.average.index eq $list_product_average[$id_product]|round}checked="checked"{/if} />
		{/section}	
	{else}
		-
	{/if}
