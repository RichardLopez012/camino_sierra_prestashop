    ...
    {if isset($product_manufacturer->id)}
      <div class="product-manufacturer">
        {if isset($manufacturer_image_url)}
          <a href="{$product_brand_url|escape:'quotes':'UTF-8'}">
            <img src="{$manufacturer_image_url|escape:'quotes':'UTF-8'}" class="img img-thumbnail manufacturer-logo" alt="" />
          </a>
        {else}
          <label class="label">{l s='Brand' d='Shop.Theme.Catalog'}</label>
          <span>
            <a href="{$product_brand_url|escape:'quotes':'UTF-8'}">{$product_manufacturer->name|escape:'html':'UTF-8'}</a>
          </span>
        {/if}
      </div>
    {/if}
