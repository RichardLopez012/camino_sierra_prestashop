        ...
        <ul id="group_{$id_attribute_group|escape:'html':'UTF-8'}">
          {foreach from=$group.attributes key=id_attribute item=group_attribute}
            <li class="pull-xs-left input-container">
              ...
              <span
                {if $group_attribute.html_color_code}class="color color_{$group_attribute.name|escape:'html':'UTF-8'}" style="background-color: {$group_attribute.html_color_code nofilter}" {/if}
